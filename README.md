...
**Domain:** `ESG / Marketing Compliance` · **Suite:** `Legal & Compliance Suite` · **Accent:** `#c084fc`
...
**Green Claims Checker** is a standalone, browser-based AI workflow app for **ESG / Marketing Compliance**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `marketing_claims`<br>Marketing claims | text | Yes | — | Affects input: Marketing claims. |
...
| `market_jurisdiction`<br>market/jurisdiction | select | No | United States | Affects customization: market/jurisdiction. |
| `claim_type`<br>claim type | text | No | — | Affects customization: claim type. |
...
| `brand_tone`<br>brand tone | text | No | — | Affects customization: brand tone. |
| `evidence_standard`<br>evidence standard | text | No | — | Affects customization: evidence standard. |
| `rewrite_style`<br>rewrite style | select | No | — | Affects customization: rewrite style. |
...
- **market/jurisdiction**: United States, United Kingdom, European Union, Egypt, UAE, Saudi Arabia, Custom
...
uvicorn app.main:app --host 127.0.0.1 --port 9125
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9125`. |
...
- Export download
- Job history
- Regulatory reference library optional
...
```bash
curl http://127.0.0.1:9125/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9125/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9125 uvicorn app.main:app --host 0.0.0.0 --port 9125
```
...
- `plugins/green-claims-checker.json` for plugin metadata.
